...
async function testPackagedVersion() {
    ...
    try {
        ...
        console.log(`   Unlock hash: ${String(unlockHash).substring(0, 50)}...`);
        
        // Verify hash
        const isValid = await KineticKeys.verifyUnlockHash(unlockHash, 'test-password');
        ...
        const testPayload = JSON.stringify({ data: "staging test payload", id: uniqueId });
        const systemSecret = "staging-secret";
        const voucher = KineticKeys.createVoucher(testPayload, unlockHash, systemSecret);
        const decryptedPayload = await KineticKeys.decryptVoucher(voucher, 'test-password', unlockHash, systemSecret);
        
        console.log(`   Original payload: ${testPayload}`);
        console.log(`   Decrypted payload: ${decryptedPayload}`);
        console.log(`   Match: ${testPayload === decryptedPayload ? '✅' : '❌'}`);
